...
{
    public int index;
    private ScrollSnap snap;

    private void Start()
    {
        snap = GetComponentInParent<ScrollSnap>();
    }
    ...
    void OnClick ()
	{
        ...
        if(snap != null)
        {
            if(index.Equals(snap.GetIndex()))
            {
                Debug.Log("TEST" + snap.GetIndex());
                //SongSelectScene.GetInstance.SongSelectSceneMove();
            }
        }
