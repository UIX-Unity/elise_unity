...
public class UIPrefabTool : EditorWindow
{
    ...
    UnityEngine.GameObject draggedObject
	{
		get
		{
			if (DragAndDrop.objectReferences == null) return null;
			if (DragAndDrop.objectReferences.Length == 1) return DragAndDrop.objectReferences[0] as UnityEngine.GameObject;
			return null;
		}
		set
		{
	...
	void AddItem (UnityEngine.GameObject go, int index)
	{
	...
	{
        UnityEngine.GameObject go = NGUIEditorTools.GUIDToObject<UnityEngine.GameObject>(guid);
	...
	Item FindItem (UnityEngine.GameObject go)
	{
	...
	public void RegenerateTexture (UnityEngine.GameObject prefab, UISnapshotPoint point)
	{
		for (int i = 0; i < mItems.size; ++i)
		{
			Item item = mItems[i];
	...
	Item CreateItemByPath (string path)
	{
		if (!string.IsNullOrEmpty(path))
		{
			...
			if (!string.IsNullOrEmpty(guid))
			{
                UnityEngine.GameObject go = AssetDatabase.LoadAssetAtPath(path, typeof(UnityEngine.GameObject)) as UnityEngine.GameObject;
				Item ent = new Item();
				ent.prefab = go;
				ent.guid = guid;
				GeneratePreview(ent, null);
	...
	static bool SetupPreviewForUI (Camera cam, UnityEngine.GameObject root, UnityEngine.GameObject child, UISnapshotPoint point)
	{
	...
	static bool SetupPreviewFor3D (Camera cam, UnityEngine.GameObject root, UnityEngine.GameObject child, UISnapshotPoint point)
	{
        ...
        UnityEngine.GameObject lightGO = NGUITools.AddChild(root);
	...
	static void SetupSnapshotCamera (UnityEngine.GameObject go, Camera cam, UISnapshotPoint point)
	{
	...
	static void SetupSnapshotCamera (UnityEngine.GameObject go, Camera cam, float objectSize, float orthoSize, float near, float far)
	{
	...
	void OnGUI ()
	{
        ...
        UnityEngine.GameObject dragged = draggedObject;
