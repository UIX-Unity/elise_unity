...
public class UIFontMaker : EditorWindow
{
	...
	void OnGUI ()
	{
        ...
        UnityEngine.GameObject go = AssetDatabase.LoadAssetAtPath(prefabPath, typeof(UnityEngine.GameObject)) as UnityEngine.GameObject;
		Object prefab = null;
		string fontName;
		...
		if (go == null || go.GetComponent<UIFont>() == null)
		{
			...
			go = new UnityEngine.GameObject(fontName);
		...
		if (prefab != null)
		{
			...
			// Select the atlas
			go = AssetDatabase.LoadAssetAtPath(prefabPath, typeof(UnityEngine.GameObject)) as UnityEngine.GameObject;
			uiFont = go.GetComponent<UIFont>();
		}
