...
//				EditorGUILayout.HelpBox("Use (Tools\\Parse Terms) to find how many times each of the Terms are used", UnityEditor.MessageType.Info);
			...
			if (nUses >= 0) 
			{
				...
				else 
				{
                    if (GUI.Button(new Rect(20, YPosMin + 2, 30, mRowSize), nUses.ToString(), "toolbarbutton"))
                    {
                        ...
                        List<UnityEngine.GameObject> selGOs = new List<UnityEngine.GameObject>();
		...
		{
            List<UnityEngine.GameObject> SelectedObjs = FindObjectsUsingKey(Key);
        ...
        List<UnityEngine.GameObject> FindObjectsUsingKey(string Key)
        {
            List<UnityEngine.GameObject> SelectedObjs = new List<UnityEngine.GameObject>();
