...
public class ListPositionCtrl : MonoBehaviour
{
	...
	void Start()
	{
        canvasDistance = Screen.width / 3f;
    ...
    private void LateUpdate()
    {
        ...
        {
            if (listBoxes[i].transform.localPosition.y > -100 &&
                listBoxes[i].transform.localPosition.y < 100)
            {
                listBoxes[i].GetComponent<Text>().enabled = (false);
                title.text = listBoxes[i].GetComponent<Text>().text;
            }          
            else       
            {
                listBoxes[i].GetComponent<Text>().enabled = (true);
            }
        }
    }
